...
const HorizontalProjectCard = ({ title, techStack, year, description, githubLink }) => {
  return (
    <motion.div 
      className="bg-black-200/60 p-6 rounded-2xl border border-tertiary/40 min-w-[320px] max-w-[320px] h-[280px] flex flex-col shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 group backdrop-blur-sm"
    ...
    >
      <div className="mb-4 flex justify-between items-start">
        <div className="flex-1 pr-2">
          <h3 className="text-white font-bold text-[18px] mb-2 leading-tight">{title}</h3>
          <p className="text-tertiary text-[13px] font-medium">{techStack}</p>
        </div>
        {githubLink && (
          <motion.a 
            href={githubLink} 
      ...
      <p className="text-secondary text-[13px] mb-3 font-light opacity-80">{year}</p>

      <p className="text-white-100 text-[14px] flex-grow leading-relaxed opacity-90">
        {description}
      </p>
